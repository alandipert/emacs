...
`prettify-text-compose-predicate'."
...
`prettify-text-unprettify-at-point'.
...
tokens and keys associated with passwords."
  ...
  (let ((identifier 'auth-source-reveal-regexp)) ; The identifier symbol.
    (if auth-source-reveal-mode
        ...
        (when (prettify-text-add-prettification
               identifier
          ...
          (unless prettify-text-unprettify-at-point
            (auth-source-do-warn
             "Please set `%s' to _see_ passwords at point"
             'prettify-text-unprettify-at-point)))
      ;; Else, when disabling, remove the prettifications for our identifier.
      (prettify-text-remove-prettifications identifier))))
